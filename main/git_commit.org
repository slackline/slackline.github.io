:PROPERTIES:
:ID:       f4f7b185-7263-418d-bb53-e5280e2b9068
:mtime:    20251223100800 20251208204145 20250423135552 20250304204632 20241025200654
:ctime:    20241025200654
:END:
#+TITLE: Git Commit
#+FILETAGS: :git:

* Secrets

Sometimes you have secrets such as API keys that may reside in files within your repository. Committing and pushing
these to a [[id:28ffcc43-9dff-4de8-a211-277c5346a642][Forge]] is a bad idea, they then become public and whilst it is possible to remove them using tools such as [[https://rtyley.github.io/bfg-repo-cleaner/][BFG
Repo Cleaner]] this is a bit of a pain and still leaves you having exposed your secrets for a period of time. Better to
avoid in the first place.

** [[https://github.com/gitleaks/gitleaks][GitHub - gitleaks/gitleaks: Find secrets with Gitleaks 🔑]]

A useful tool with both a [[https://github.com/gitleaks/gitleaks#github-action][GitHub Action]] and [[https://github.com/gitleaks/gitleaks#pre-commit][Pre-Commit]] hook that allows you to check that secrets have not been added to
your repository.

You'd be best of using the [[https://github.com/gitleaks/gitleaks#pre-commit][Pre-Commit]] hook to ensure you don't commit and push changes.

* Conventions

** Conventional Commits

[[https://www.conventionalcommits.org/en/v1.0.0/][Conventional Commits]] and [[https://conventionalcomments.org/][Conventional Comments]] are two frameworks to provide structure to commit comments (and
issues). They could be extended to [[id:d8720ab9-a43e-45e4-834e-e4c5ce4f4e43][Git Notes]] without much modification.

There is however a case for [[https://sumnerevans.com/posts/software-engineering/stop-using-conventional-commits/][just using scope]]. The author suggests the ~type~ should be inferred from the message and
that the promises the meta data adds have failed to deliver. They also cite popular projects (Linux kernel, Git,
FreeBSD, Go and others) that simply use scope successfully.

** Commits

** Comments

* Links

+ [[https://www.conventionalcommits.org/en/v1.0.0/][Conventional Commits]]
+ [[https://dev.to/craicoverflow/enforcing-conventional-commits-using-git-hooks-1o5p][Enforcing Conventional Commits using Git Hooks]]
+ [[https://github.com/RomuloOliveira/commit-messages-guide/tree/master][commit-messages-guide: A guide to understand the importance of commit messages and how to write them well]]
+ [[https://github.com/carloscuesta/gitmoji-cli][carloscuesta/gitmoji-cli: A gitmoji interactive cli tool for using emojis on commits. 💻]]

** Tools

+ [[https://github.com/gitleaks/gitleaks][Git Leaks]]
+ [[https://rtyley.github.io/bfg-repo-cleaner/][BFG Repo Cleaner]]
+ [[https://conventionalcomments.org/][Conventional Comments]]

** Blogs

+ [[https://sumnerevans.com/posts/software-engineering/stop-using-conventional-commits/][Stop Using Conventional Commits]]
+ [[https://brennan.day/every-commit-a-sentence-git-commit-messages-for-bloggers/][Every Commit A Sentence: Git Commit Messages for Bloggers · brennan.day]]
+ [[https://www.jvt.me/posts/2026/08/17/hand-write-commits/][Why I still hand write my commit messages · Jamie Tanna | Software Engineer]]
+ [[https://cbea.ms/git-commit/][How to Write a Git Commit Message]]
