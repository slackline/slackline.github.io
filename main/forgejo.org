...
:mtime:    20250917073053 20250913150944 20250322195213 20250321090032 20250314091326 20250214202539 20250212211450 20241221213704 20240708222139 20240708205417 20240309204131 20240215065336 20240214220237 20240214203455 20240213073417 20240211204231 20240211163700 20240211140707
:ctime:    20240211140707
:END:
#+TITLE: ForgeJo
#+FILETAGS: :git:forges:selfhosting:

[[https://forgejo.org/][Forgejo]] is a fork of [[https://gitea.io][Gitea]] and is used by [[id:240b85db-27c9-4848-9d3f-c3be841a193b][Codeberg]] to run their site.
...
The installation  are for Gitea but I simply substituted every instance of ~gitea~ for ~foregjo~. I opted
...
I found that the ~forgejo~ account was by default "expired" and needed [[https://wiki.archlinux.org/title/Forgejo#Configure_SSH][remove the expiration]].

#+begin_example
chage -E -1 forgejo
#+end_example

Then enable ~UsePAM yes~ in the ~/etc/ssh/sshd_config~ (and restart the service).
...
#+begin_src bash
git remote add forgejo ssh://forgejo@forgejo.your.domain:12345/<user_account>/test.git
#+end_src
...
See [[id:d25bfba9-1460-4c15-bf86-dbabffadc24e][ForgeJo Runners]].
...
You can easily backup Forgejo to a zip using [[https://forgejo.org/docs/latest/admin/command-line/#dump][~forgejo dump command~]], but only if ~forgejo~ is running at the time
(i.e. not from a ~chroot~ environment).
...
+ [[https://spiffyk.cz/blog/git-pandoc/][Diffing Word and Writer documents with Git – Oto Šťáva]]

** Blogs

+ [[https://jorijn.com/en/blog/leaving-github-for-forgejo/][Leaving GitHub for ForgeJo]]
