...
:mtime:    20240607080242 20240606223325 20240605074417 20240602222241 20240601082743 20230730204045 20230730112159
...
I found on initially starting Fittrackee and creating a user account that I was then unable to login due to "Invalid
Credentials" and attributed that to not having verified my email address as I hadn't configured an email account. I
sought to remedy this by installing [[https://wiki.archlinux.org/title/Postfix][postfix]] which also required [[https://wiki.archlinux.org/title/Obtain_a_certificate][obtaining a certificate]].

On review I opted to use Gmail's SMTP service, this required creating an [[https://www.zdnet.com/article/gmail-app-passwords-what-they-are-how-to-create-one-and-why-to-use-them/][App Password]] and configuring the ~EMAIL_URL~ to
use this via the ~.env~ configuration file (this [[https://mailtrap.io/blog/gmail-smtp/][article]] was also useful).

#+begin_src
export EMAIL_URL=smtp://<gmail_user>:<app_password>@smtp.gmail.com:465
#+end_src
** Set up your ~\~/.env~
...
I found on starting that I had to register for an account, this in turn requires an email server and Redis to work
properly.

For email I opted to use [[https://wiki.archlinux.org/title/Postfix][Postfix - ArchWiki]]

#+begin_src bash
pacman -Syu postfix postfix-lmdb
src

Settings are under ~/etc/postfix/main.cf~ and are really well documented. Key fields I changed, including setting
aliases for ~root~ as advised in ~/etc/postfix/aliases~.
...
Redis installation was straight-forward I followed the [[https://wiki.archlinux.org/title/Redis][Redis - ArchWiki]]
...
Useful commands for working with the database that underpins Fittrackee.
...
One day I might sit down and learn SQL in greater detail.

#+begin_src sql
-- List all fields in table users
SELECT * FROM users;
-- Delete a row
