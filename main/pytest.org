...
Running tests takes time and ideally tests should run as quickly as possible. You can check how long it takes for a test
suite to run using `pytest --duration=0` which reports where the time taken to process lasts.
