...
You can configure pytest within a projects directory via a choice of files ~setup.cfg~ is sufficient for simple use
cases but its recommended more complex configurations are made via ~pyproject.toml~.

#+begin_example toml
[tool.pytest]
minversion = "9.0"
addopts = ["-ra", "--showlocals", "--mpl", "--cov", "-m", "not slow"]
strict = true
filterwarnings = [
  "error",
]
...
markers = [
    "slow: marks tests as end-to-end: tests ran weekly via Actions",
]
#+end_example
...
You can parameterise fixtures using the `pytest.fixture(params=<something>)`
#+CAPTION: Parameterising fixtures
#+NAME: pytest-parameterise-fixtures
#+begin_src python :eval no
import pytest

@pytest.fixture(params=[1, 2])
def a_number(request):
...
An alternative if you do not want to test the values of parameters that pass then you can use the following which I
found on [[https://borutzki.github.io/2026/01/15/how-to-parametrize-exception-testing-in-pytest.html][this blog post]].

#+CAPTION: Parameterising tests, some of which pass, others fail.
#+NAME: pytest-fixture-pass-fail-alternative
#+BEGIN_SRC python :eval no
...
run. This can be done using [[https://docs.pytest.org/en/stable/example/markers.html#marking-test-functions-and-selecting-them-for-a-run][@pytest.mark]] fixture to group tests and using the `pytest -m <name>` to select them.
...
You can then selectively run tests using the `slow` mark.
...
Your packages ~pyproject.toml~ which will typically be used to configure Pytest via the ~tool.pytest~ section.

If you want to normally exclude the slow test then you can include the option to do so under ~addopts~. To avoid warnings you should also register the markers and give a description.
...
If you have structured nomenclature to your tests that indicates what the tests are then you can [[https://docs.pytest.org/en/stable/example/markers.html#automatically-adding-markers-based-on-test-names][automate adding markers]]
based on these.
...
Markers are a really useful feature of Pytest that makes it possible to run subsets of tests. As [[https://docs.pytest.org/en/stable/example/markers.html#marking-platform-specific-tests-with-pytest][documented]] this can be
used to make tests platform specific, but as the above simple examples start to demonstrate they are highly flexible and
customisable.

** Coverage

The [[https://pytest-cov.readthedocs.io/en/latest/][pytest-cov]] module makes it easy to look at the coverage of your tests and is a Pytest extension of the [[https://coverage.readthedocs.io/en/latest/install.html][~coverage~]] package. It summarises which lines were run during testing and which were missed simply by specifying ~--cov~. An
additional option though is ~--cov-branch~ which looks at whether code within e.g. ~if~ clauses are run.

Configuration can be done under ~[tools.pytest.ini_options]~ and more detailed configuration should be made via
[[https://coverage.readthedocs.io/en/latest/config.html][~.coveragerc~]] configuration file for [[https://coverage.readthedocs.io/en/latest/install.html][~coverage~]].
...
[[https://syrupy-project.github.io/syrupy/][Syrupy]] is another pytest snapshot plugin. Its pretty useful and I tend to prefer it over ~pytest-regtest~.

*** Precision

Precision issues are always a perennial problem though so sometimes the values in the ~snapshot~ differ from those on
different OS's. Syrupy  has a neat way of handling this though allowing the ~matcher~ parameter to be used in
conjunction with ~replacer~ to round values.

#+begin_example python
def test_something(..., snapshot) -> None:
    """Test something."""
    results = ...
    assert results == snapshot(matcher, path_type(types=(float,), replacer=lambda data, _: round(data, 4)))
#+end_example

*** Pandas DataFrames

Syrupy [[https://github.com/syrupy-project/syrupy/issues/887][does not support Pandas dataframes]] (yet), the solution is to convert them ~to_string~
* Miscellaneous
...
against. Although this is in essence what ~pytest-testmon~ does.
