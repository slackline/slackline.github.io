...
:mtime:    20251224220620 20251214125445 20251213182206 20251212223257 20251212195343 20251212103425 20251211094111 20251211003606 20251208213404 20251208202826 20251208175553 20251207212421 20251207194725 20251207121232
...
Needed/decided to install a few extra packages based on recommendations during this install for various packages that I
...
[[id:fb505adf-5df0-4d6e-a7b1-9853044653c5][Arch AUR]] has a number of different helper packages to manage such user installs, my preference is for [[https://github.com/morganamilo/paru][Paru]].
...
LABEL=5tb-backup /mnt/usb ext4  auto,nofail,x-systemd.automount,users 0 0
#+end_example
...
I should probably have done this when creating my ~user~ account but instead opted to change it after logging in.
...
I use [[id:a4af96d2-f895-4a68-9419-d482d2850f4d][Direnv]] in many directories to automatically set the environment variables. If you do too then you may have to use
~direnv allow~ when entering each of these.
...
#+begin_example
─────┬────────────────────────────────────────────────────────────────────────────────
     │ File: systemd/system/multi-user.target.wants/snapclient.service
─────┼────────────────────────────────────────────────────────────────────────────────
   1 │ [Unit]
   2 │ Description=Snapcast client
   3 │ Documentation=man:snapclient(1)
   4 │ Wants=avahi-daemon.service
   5 │ After=network-online.target time-sync.target sound.target avahi-daemon.service
   6 │
   7 │ [Service]
   8 │ EnvironmentFile=-/etc/default/snapclient
   9 │ ExecStart=/usr/bin/snapclient --logsink=system $SNAPCLIENT_OPTS
  10 │ User=snapclient
  11 │ Group=snapclient
...
The service now starts and I get audio from the attached amp and on all other devices around my house 🔊🎶
...
Why has this happened, I've found ~rpi-armv7h~ to be fairly stable over the years and had little warning that this was
coming up.

I have subsequently mounted my old system under the recovered one (which has blocked updates of the ~linux-rpi~ kernel
beyond ~6.12.56-1~ along with all ~linux-firmware-*~ that is ~>=20251021-1~) and when updating I saw the following
message which means that maintainers know about the problem even if I can't find where it is documented/acknowledged.
...
+ [[https://archlinuxarm.org/forum/viewtopic.php?t=14734][Arch Linux ARM • View topic - No USB detected on Raspberry Pi 4 with aarch64]]
