...
[[https://github.blog/open-source/git/highlights-from-git-2-54/#h-config-based-hooks][Git 2.54]] saw the introduction of config based hooks which allow broader configuration at the system and global level as
well as local scope.
...
The ~$GIT_DIR/hooks~ jobs will run /after/ configuration based hooks.
